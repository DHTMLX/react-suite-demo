...
class CalendarEvents extends Component {
	...
	componentDidMount() {
		...
		this.calendar.events.on("dateMouseOver", (date) => this.setState({event: "dateMouseOver", item: date.toString()}));
		this.calendar.events.on("beforeChange", id => this.setState({event: "change", item: id.toString()}));
		...
		this.calendar.events.on("monthSelected", month => this.setState({event: "monthSelected", item: month + 1 + ""}));
		this.calendar.events.on("yearSelected", year => this.setState({event: "yearSelected", item: year.toString()}));
	}
		...
		return (
			<div style={{width: "100%"}}>
				...
				<div style={{display: "flex", justifyContent: "center", padding: 20}}>
					...
					<button className="button button--bordered">Item: {this.state.item ? this.state.item : ""}</button>
				</div>
			</div>
		);
	}
